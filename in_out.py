    ...
    while True:
        ...
        if right == "" and left == "":
            ...
            if x < 200:
                ...
                right, left = "", ""
                now = datetime.now()
                cv2.imwrite(f"visitors/in/{now.year%100}-{now.month}-{now.day}-{now.hour}-{now.minute}-{now.second}.jpg", frame1)
            ...
            if x > 500:
                ...
                right, left = "", ""
                now = datetime.now()
                cv2.imwrite(f"visitors/out/{now.year%100}-{now.month}-{now.day}-{now.hour}-{now.minute}-{now.second}.jpg", frame1)
